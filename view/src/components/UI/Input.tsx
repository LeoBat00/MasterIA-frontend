...
const iconSpaceBySize: Record<
  InputSize,
  { left: string; right: string; icon: number }
> = {
      ...
      >
          ...
          >
            {React.isValidElement(leftIcon)
              ? React.cloneElement(leftIcon as any, {
                  size: iconSpaceBySize[size].icon,
                })
              : leftIcon}
          </button>
        )}
        ...
        {mask ? (
          <IMaskInput
            ...
            onAccept={(val: unknown) =>
              handleChange(
                number ? String(val).replace(/\D/g, "") : String(val)
              )
            }
            id={id}
          ...
          <input
            ...
            pattern={number ? "\\d*" : undefined}
            className={clsx(
              "w-full outline-none placeholder-[#868686]/60  bg-[#12121B]",
              paddingsBySize[size],
              withLeft && iconSpaceBySize[size].left,
              withRight && iconSpaceBySize[size].right,
              variant === "underline" && "rounded-none",
        ...
        {withRight && (
          ...
          >
            {willShowPasswordToggle ? (
              showPassword ? (
                <FiEyeOff size={iconSpaceBySize[size].icon} />
              ) : (
                <FiEye size={iconSpaceBySize[size].icon} />
              )
