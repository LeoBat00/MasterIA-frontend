...
export default function StepPreferencias({ next, prev }: Props) {
  ...
  const validar = () => {
    if (!perfil.experience || !perfil.qntPessoas || !perfil.tempoJogo) return false;
    const idadeNum = parseInt(perfil.idade || "0", 10);
    return idadeNum > 0;
  };

  const handleNext = () => {
    if (!validar()) return alert("Preencha todas as preferencias.");
    next();
  };
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-2">
        <label className="flex items-center gap-2 text-[#D9E8FF]"><FiStar /> Nivel de experiencia</label>
      ...
      <div className="flex flex-col gap-2">
        <label className="flex items-center gap-2 text-[#D9E8FF]"><FiUsers /> Quantidade de jogadores</label>
        <Select
          value={perfil.qntPessoas}
          onChange={(v) => typeof v === 'string' && setPerfil({ qntPessoas: v })}
          options={[{ label: 'Selecione', value: '' }, { label: '2', value: '2' }, { label: '3', value: '3' }, { label: '4', value: '4' }, { label: '5', value: '5' }, { label: '6+', value: '6+' }]}
          placeholder="Selecione"
        />
      </div>

      <div className="flex flex-col gap-2">
        <label className="flex items-center gap-2 text-[#D9E8FF]"><FiClock /> Tempo medio de jogo</label>
        <Select
      ...
      <div className="flex flex-col gap-2">
        <label className="flex items-center gap-2 text-[#D9E8FF]"><FiCalendar /> Idade</label>
        <input
          type="number"
          value={perfil.idade}
      ...
      <div className="flex justify-between w-full mt-4">
        <Button variant="outlineGhostPurple" onClick={prev}>Voltar</Button>
