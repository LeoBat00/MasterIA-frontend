...
import FiltroEventos from './filtro';
...
import { zerarHoras } from '../util';


export default function CadastroEventos() {

    const { cidadeSelecionada, setCidadeSelecionada, filtroEventos } = useCadastroEventosStore();
    ...
    const filtrarEventos = (eventos: Evento[]): Evento[] => {
        ...
        if (filtroEventos?.codigoEvento) {
            eventosFiltrados = eventosFiltrados.filter(evento =>
                evento.cdEvento.toLowerCase().includes(filtroEventos.codigoEvento!.toLowerCase())
            );
        }
        ...
        if (filtroEventos?.inPeriodo) {
            switch (filtroEventos.inPeriodo) {
                ...
                case EventoPeriodo.MES: {
                    const hoje = new Date();
                    const inicioMes = zerarHoras(new Date(hoje.getFullYear(), hoje.getMonth(), 1));
                    const fimMes = zerarHoras(new Date(hoje.getFullYear(), hoje.getMonth() + 1, 0)); // último dia do mês
    ...
    return (
        <div className='min-h-screen bg-[#1C172E]'>
            ...
            {!cidadeSelecionada ?
                ...
                (
                    <div className='mx-50 p-8'>
                        ...
                        <h1 className="titulo-pagina mt-5">Eventos acontecendo <br></br> em <span className='text-[#616EFF] font-bold'> {cidadeSelecionada?.toUpperCase()} </span></h1>
