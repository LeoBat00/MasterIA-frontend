...
export default function RegistroPage() {
  ...
  const validarPasso1 = () => {
    ...
    if (!camposForm.email?.trim()) e.email = "O campo e-mail é obrigatório.";
      ...
      if (!emailRegex.test(camposForm.email))
        e.email = "O campo e-mail deve ser um e-mail válido.";
    }

    // senha
    if (!camposForm.senha?.trim()) e.senha = "O campo senha é obrigatório.";
    else if ((camposForm.senha?.length ?? 0) < MIN_SIZE_SENHA)
      e.senha = "O campo senha deve ter no mínimo 6 caracteres.";

    // confirmar senha
    if (!camposForm.confirmarSenha?.trim())
      e.confirmarSenha = "O campo confirmar senha é obrigatório.";
    else if (camposForm.senha !== camposForm.confirmarSenha)
  ...
  const validarPasso2 = () => {
    ...
    else if ((camposForm.nome?.length ?? 0) < 3)
      e.nome = "O campo nome deve ter no mínimo 3 caracteres.";
    else if ((camposForm.nome?.length ?? 0) > MAX_SIZE_NOME)
      e.nome = `O campo nome deve ter no máximo ${MAX_SIZE_NOME} caracteres.`;

    // cpf/cnpj
    if (!camposForm.cpfCnpj?.trim())
      e.cpfCnpj = "O campo CPF/CNPJ é obrigatório.";
    else {
      const d = onlyDigits(camposForm.cpfCnpj);
      if (d.length !== 11 && d.length !== 14)
        e.cpfCnpj = "O campo CPF/CNPJ deve ter 11 (CPF) ou 14 (CNPJ) dígitos.";
    ...
    if (camposForm.telefone) {
      ...
      if (cleaned.length < 10 || cleaned.length > 11)
        e.telefone = "O campo telefone deve ter 10 ou 11 dígitos.";
    }
          ...
          {step === 1 ? (
            <>
              ...
              <button
                type="submit"
                disabled={loading}
                className="bg-[var(--color-button-primary)] text-black py-2 cursor-pointer rounded hover:opacity-90 mt-4 disabled:opacity-60"
              >
            ...
            <>
              ...
              <div className="flex gap-3 mt-2">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="flex-1 bg-[var(--color-button-primary)] cursor-pointer rounded-[8px] text-black py-2 mt-4  hover:opacity-90 disabled:opacity-60"
                >
